# HTTP::API::Client — Project Direction

This document defines the design direction for `HTTP::API::Client`. New features should be evaluated against these principles so that the distribution stays small, predictable, and useful as a long-lived foundation for Perl API clients.

## Goal

> **HTTP::API::Client is a small, dependency-light foundation for building production-quality HTTP API clients in Perl.**
...
`HTTP::API::Client` is an API-client layer, not an HTTP stack.
...
These may be implemented by separate distributions built on top of `HTTP::API::Client` where appropriate.
