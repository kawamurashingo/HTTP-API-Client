...
`HTTP::API::Client` is an API-client layer, not an HTTP stack. The transport boundary is a supported extension point so alternate HTTP implementations can be used without changing API-specific client code.
...
`HTTP::API::Client` converts this transport response into `HTTP::API::Client::Response`.

## Failures

A transport may throw an exception for connection, TLS, timeout, DNS, or other transport failures. Such failures are normalized into `HTTP::API::Client::Error` with `category => 'transport'` and participate in the configured retry policy.
